...
class UnitConverterTests(unittest.TestCase):
    ...
    def test_temperature_conversions(self):
        self.assertAlmostEqual(unidades.converter(32, "Temperatura", "°F", "°C"), 0)
        self.assertAlmostEqual(unidades.converter(100, "Temperatura", "°C", "°F"), 212)
        self.assertAlmostEqual(unidades.converter(273.15, "Temperatura", "K", "°C"), 0)
    ...
    def test_round_trip_for_every_unit(self):
        for categoria, itens in unidades.CATEGORIAS.items():
            ...
            for destino in itens:
                convertido = unidades.converter(12.345, categoria, origem, destino)
                retorno = unidades.converter(convertido, categoria, destino, origem)
...
class ProjectAssistantTests(unittest.TestCase):
    def test_loads_are_prepared_before_static_check(self):
        rota = projetos.recomendar_rota(projetos.OBJETIVOS[1], projetos.DADOS_DISPONIVEIS[0])
        self.assertEqual(rota.chave, "cargas")
        self.assertEqual(
            projetos.sequencia_recomendada(projetos.OBJETIVOS[1], projetos.DADOS_DISPONIVEIS[0]),
            ["casos_carga", "cargas", "estatica", "analise_sensibilidade"],
        )

    def test_known_cycle_routes_to_fatigue(self):
        rota = projetos.recomendar_rota(projetos.OBJETIVOS[3], projetos.DADOS_DISPONIVEIS[2])
    ...
    def test_rotas_include_casos_carga_and_sensibilidade(self):
        ...
        self.assertEqual(projetos.ROTAS["casos_carga"].pagina, "app_pages/casos_carga.py")
        self.assertEqual(
