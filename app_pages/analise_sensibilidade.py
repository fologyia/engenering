...
with st.expander("Partir de um cálculo já registrado", expanded=False):
    ...
    if registros_compativeis:
        ...
        origem_id = st.selectbox(
            "Registro de origem",
            list(mapa_registros),
            format_func=lambda valor: mapa_registros[valor],
        )
        if st.button("Carregar entradas rastreadas", icon=":material/input:"):
            registro, sugestao = next(
                item for item in registros_compativeis if item[0]["id"] == origem_id
            )
            ...
            st.success(
                "Entradas disponíveis carregadas. Revise incertezas e critério antes de calcular."
            )
            st.rerun()
    else:
        st.caption(
            "O projeto ativo ainda não possui análise estática ou de fadiga compatível com importação automática."
        )

if (
    "sens_modelo_id" not in st.session_state
    or st.session_state["sens_modelo_id"] not in mapa_modelos
):
...
with st.form(f"form_sensibilidade_{modelo_id}"):
    ...
    dados_editados = st.data_editor(
        ...
        column_config={
            ...
            "Distribuição": st.column_config.SelectboxColumn(
                "Distribuição", options=list(DISTRIBUICOES), required=True
            ),
        },
    ...
    variacao_oat = c1.number_input(
        "Variação OAT (± %)", min_value=0.1, max_value=90.0, value=10.0, step=1.0
    )
    amostras = c2.number_input(
        "Amostras Monte Carlo", min_value=200, max_value=50_000, value=3_000, step=500
    )
    semente = c3.number_input(
        "Semente reproduzível", min_value=0, max_value=2_147_483_647, value=42, step=1
    )
    ...
    operador = c2.selectbox(
        "Resultado deve ser",
        ["<=", ">="],
        index=1 if operador_padrao == ">=" else 0,
    ...
    calcular = st.form_submit_button(
        "Executar sensibilidade e incerteza", type="primary", icon=":material/monitoring:"
    )

if calcular:
    try:
        entradas = {
            str(linha["chave"]): float(linha["Nominal"]) for _, linha in dados_editados.iterrows()
        }
        configuracao = {
            str(linha["chave"]): {
...
if resultado and resultado.get("modelo_id") == modelo_id:
    ...
    m2.metric(
        "Entrada governante",
        governante["variavel"],
        f"{governante['impacto_percentual']:.1f}% de faixa",
    )
    m3.metric("Faixa P05–P95", f"{mc['p05']:.4g} – {mc['p95']:.4g}")
    m4.metric(
        "Risco de não atender", "Não avaliado" if probabilidade is None else f"{probabilidade:.2f}%"
    )
    ...
    grafico_ranking = (
        ...
        .encode(
            ...
            color=alt.Color(
                "direcao_critica:N", title="Piora ao", scale=alt.Scale(range=["#7C3AED", "#D97706"])
            ),
            tooltip=[
    ...
    variavel_curva = st.selectbox(
        "Entrada explorada", list(mapa_variaveis), format_func=lambda valor: mapa_variaveis[valor]
    )
    ...
    grafico_curva = (
        ...
        .encode(
            ...
            y=alt.Y(
                "saida:Q",
                title=f"{modelo.saida} ({modelo.unidade_saida})",
    ...
    h2.dataframe(
        correlacoes.rename(columns={"variavel": "Entrada", "correlacao": "Correlação"})[
            ["Entrada", "Correlação"]
        ],
        hide_index=True,
        width="stretch",
    ...
    with st.expander("Tabela completa e premissas do resultado"):
        st.dataframe(
            ranking_df.rename(
                columns={
                    "variavel": "Entrada",
                    "nominal": "Nominal",
                    "saida_menos": "Saída em −Δ",
                    "saida_mais": "Saída em +Δ",
                    "impacto_percentual": "Impacto (%)",
                    "elasticidade": "Elasticidade",
                    "direcao_critica": "Direção crítica",
                }
            )[
                [
                    "Entrada",
        ...
        st.write(
            f"Amostras válidas: {mc['amostras_validas']} de {mc['amostras_solicitadas']}; semente {mc['semente']}."
        )
    ...
    if probabilidade is not None:
        status = (
            "Atende"
            if atende_nominal and probabilidade <= 5.0
            else ("Atenção" if atende_nominal else "Não atende")
        )
    conclusao = (
        f"{governante['variavel']} é a entrada mais influente no intervalo OAT de ±{oat['variacao_percentual']:.1f}%. "
        + (
            "Critério probabilístico não definido."
            if probabilidade is None
            else f"Probabilidade estimada de não atendimento = {probabilidade:.2f}%."
        )
    )
    registro = construir_registro_tecnico(
            ...
            "ranking_sensibilidade": [
                {
                    ...
                    "impacto_percentual": item["impacto_percentual"],
                    "elasticidade": item["elasticidade"],
                    "direcao_critica": item["direcao_critica"],
                }
        ...
        referencias=[
            "Registro de origem e base de cálculo do projeto devem ser conferidos antes da decisão."
        ],
        conclusao=conclusao,
